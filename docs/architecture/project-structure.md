...
- `agents/personal/guides/`: detailed feature guides that a helper serves on demand, such as `npm run --silent focus -- guide` and `npm run --silent playbook -- guide`. They are not rendered into workspaces and do not count against the project-doc budget.
